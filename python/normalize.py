...
for i in range(0, len(trades_files) - 1):
    ...
    bids = np.array(depth['bids']).astype(Decimal)
    asks = np.array(depth['asks']).astype(Decimal)
    ...
    trades = np.array([pick_price_quantity(p) for p in trades_raw]).astype(Decimal)
    # print(trades)
